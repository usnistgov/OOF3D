// -*- C++ -*-
// $RCSfile: fixillegal.h,v $
// $Revision: 1.1.2.3 $
// $Author: langer $
// $Date: 2014/12/14 01:07:52 $

/* This software was produced by NIST, an agency of the U.S. government,
 * and by statute is not subject to copyright in the United States.
 * Recipients of this software assume all responsibilities associated
 * with its operation, modification and maintenance. However, to
 * facilitate maintenance we ask that before distributing modified
 * versions of this software, you first contact the authors at
 * oof_manager@nist.gov. 
 */

#include <oofconfig.h>

#ifndef FIXILLEGAL_H
#define FIXILLEGAL_H

#include "common/random.h"
#include "engine/cskeleton2_i.h"
#include "engine/cskeletonmodifier.h"

class CSkeletonNode;

class FixIllegal : public CSkeletonModifier {
protected:
  int nguilty, ndone;
  OOFRandomNumberGenerator rand;
  CSkeletonNodeSet neighborNodes;

public:
  FixIllegal() {}; 
  virtual ~FixIllegal() {};
  int getNGuilty() {return nguilty;};
  int getNDone() {return ndone;};
  CSkeletonBase *apply(CSkeletonBase *skeleton);
  void smoothIllegalElements(CDeputySkeleton *newSkeleton, 
			     CSkeletonElementSet &bad_set, 
			     CSkeletonElementVector &baddies);
  void smoothTheNode(CDeputySkeleton *newSkeleton,
		     CSkeletonNode *node,
		     CSkeletonElementSet &bad_set, 
		     CSkeletonElementVector &baddies);

};


#endif


