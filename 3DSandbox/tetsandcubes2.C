...
#define IMAGESIZE 2		// number of checkerboard squares on a side
#define MESHSIZE  1		// number of subcubes on a side for the tet mesh
...
// Color for the tetrahedral mesh
#define MESH_R 1.0
#define MESH_G 0.0
#define MESH_B 0.0
#define MESH_A 1.0
...
vtkSmartPointer<vtkUnstructuredGrid> makeMesh(unsigned int n) {
  ...
  for(int i=0; i<n; i++) {			 // y dimension
	...
	if((i+j+k)%2 == 0) {
	  ...
	  // Drawing only some of the tets makes it easier to see
	  // what's going on.
...
int main(int argc, char *argv[]) {
  ...
  double fact, units;
  vtkMapper::GetResolveCoincidentTopologyPolygonOffsetParameters(fact, units);
  std::cerr << "GetResolveCoincidentTopologyPolygonOffsetParameters: "
	    << fact << " " << units << std::endl;
  vtkMapper::GetResolveCoincidentTopologyLineOffsetParameters(fact, units);
  std::cerr << "GetResolveCoincidentTopologyLineOffsetParameters: "
	    << fact << " " << units << std::endl;
  vtkMapper::GetResolveCoincidentTopologyPointOffsetParameter(fact);
  std::cerr << "GetResolveCoincidentTopologyPointOffsetParameter: "
	    << fact << std::endl;
  // vtkMapper::SetResolveCoincidentTopologyToShiftZBuffer();
  // vtkMapper::SetResolveCoincidentTopologyZShift(10);
  ...
  //imageActor->GetProperty()->SetRepresentationToSurface();
  imageActor->GetProperty()->SetRepresentationToWireframe();
  imageActor->GetProperty()->SetLineWidth(5);
  renderer->AddViewProp(imageActor);
  imageMapper->SetInputData(image);
  imageMapper->SetRelativeCoincidentTopologyLineOffsetParameters(-3, -3);
  ...
  //meshMapper->SetRelativeCoincidentTopologyPolygonOffsetParameters(-3, -3);
  meshMapper->SetRelativeCoincidentTopologyLineOffsetParameters(-4, -4);
