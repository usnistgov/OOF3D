	      ...
	      vtkSmartPointer<vtkRenderer> renderer)
{
  ...
  // vtkSmartPointer<vtkCellData> cdata = image->GetCellData();
  // vtkSmartPointer<vtkPointData> pdata = image->GetPointData();
  // cdata->SetScalars(pdata->GetScalars());
  // pdata->SetScalars(0);
  ...
  if(!clipping) {
    ...
    vtkSmartPointer<vtkPointDataToCellData> pt2cell =
      vtkSmartPointer<vtkPointDataToCellData>::New();
    pt2cell->PassPointDataOn();
    pt2cell->SetInputConnection(image->GetProducerPort());

    // vtkSmartPointer<vtkClipVolume> clipper =
    //   vtkSmartPointer<vtkClipVolume>::New();
    // clipper->SetClipFunction(plane);
    // clipper->Mixed3DCellGenerationOn();
    // clipper->SetInputConnection(pt2cell->GetOutputPort());
    // vtkSmartPointer<vtkUnstructuredGrid> ug = clipper->GetOutput();

    vtkSmartPointer<vtkExtractGeometry> extractor = 
      vtkSmartPointer<vtkExtractGeometry>::New();
    extractor->SetImplicitFunction(plane);
    extractor->SetInputConnection(pt2cell->GetOutputPort());
    vtkSmartPointer<vtkUnstructuredGrid> ug = extractor->GetOutput();
