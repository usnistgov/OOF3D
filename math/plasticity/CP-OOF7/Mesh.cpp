...
Mesh::Mesh(std::istream &inFile){
    ...
    int nodeNum , nmat;
    double cx , cy , cz;
    inFile >> nnode >> nelem >> tnode ;
    for (int i = 0 ; i< tnode ; i++){
        inFile >> nodeNum >> cx >> cy >> cz;
        Node* nod = new Node(nodeNum,cx,cy,cz);
    ...
    for (int j = 0 ; j < nelem ; j++){
        ...
        inFile >> ielem ;
        elconnectivities.clear();
        nodes.clear();
        for (int k = 0 ; k < nnode ; k++){
            inFile >> elc;
            elconnectivities.push_back(elc);
        }
        ...
        Element *el = new FourNodeTetrahedra(nelem,nnode,nodes);
        ellist.push_back(el);
    ...
    for (int i = 0 ; i < zelement ; i++){
        for (int j = 0 ; j < yelement ; j++){
            for (int k = 0 ; k < xelement ; k++){
                ...
                std::cout<<std::endl;
                if (nnode = 8){
                    Element *el = new EightNodeHexahedral(nelem,nnode,nodes);
                    ellist.push_back(el);
                }
