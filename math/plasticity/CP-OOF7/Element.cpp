...
#include "Element.hpp"
Element::Element(int id,int node,std::vector<Node*> nodelist) {
...
void Element::make_linear_system(Mesh* mesh){
    
    gptable.clear();
    gausspts();
    
    material->begin_element(this,mesh);
    
    material->make_linear_system(this,mesh);
...
//#########################################################################################################
//#########################################################################################################
//################## dN/dX with respect to the Reference configuration ####################################
//#########################################################################################################
//#########################################################################################################

double Element::dshapefnRef(double xi,double zeta,double mu,int node_idx,int refcomp){
    
    if (xi!=xi_evaled || zeta!=zeta_evaled || mu!=mu_evaled) {
        jmtx_cache = dshapefnRefmtx(xi,zeta,mu);
    }
    return jmtx_cache[node_idx][refcomp];
}

std::vector<std::vector<double>> Element::dshapefnRefmtx(double xi,double zeta,double mu){
    ...
    *jacobian[0][2]*jacobian[1][1];
    ...
    std::vector<std::vector<double>> dfdreft;
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//%%%%%%%%%%%%%%%%%%%%%%%%%%% dN/dx with respect to the current configuration %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

double Element::dshapefn_current(double xi,double zeta,double mu,int node_idx,int refcomp){
    
    if (xi!=xi_evaled || zeta!=zeta_evaled || mu!=mu_evaled) {
        jmtx_cache_current = dshapefn_currentmtx(xi,zeta,mu);
    }
    return jmtx_cache_current[node_idx][refcomp];
}

std::vector<std::vector<double>> Element::dshapefn_currentmtx(double xi,double zeta,double mu){
    
    
    
    jacobianmtx_current(xi,zeta,mu);
...
void Element::jacobianmtx_current(double xi,double zeta,double mu){
    ...
    for (int i = 0 ; i < nnode ; i++){
        jacobian[0][0] += dsfns[i][0]*(nodes[i]->coorx+nodes[i]->fields[0]->value[0]);
        jacobian[0][1] += dsfns[i][0]*(nodes[i]->coory+nodes[i]->fields[0]->value[1]);
        jacobian[0][2] += dsfns[i][0]*(nodes[i]->coorz+nodes[i]->fields[0]->value[2]);
        
        jacobian[1][0] += dsfns[i][1]*(nodes[i]->coorx+nodes[i]->fields[0]->value[0]);
        jacobian[1][1] += dsfns[i][1]*(nodes[i]->coory+nodes[i]->fields[0]->value[1]);
        jacobian[1][2] += dsfns[i][1]*(nodes[i]->coorz+nodes[i]->fields[0]->value[2]);
        
        jacobian[2][0] += dsfns[i][2]*(nodes[i]->coorx+nodes[i]->fields[0]->value[0]);
        jacobian[2][1] += dsfns[i][2]*(nodes[i]->coory+nodes[i]->fields[0]->value[1]);
        jacobian[2][2] += dsfns[i][2]*(nodes[i]->coorz+nodes[i]->fields[0]->value[2]);
    }
    
    
}

double Element::jacobian_current(double xi,double zeta,double mu){
