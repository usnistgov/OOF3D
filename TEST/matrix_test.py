...
def loadMatrix(filename, fortran=False, symmetric=False):
    phile = file(os.path.join(datadir, filename), 'r')
...
def loadVector(filename):
    ...
    vector.load(os.path.join(datadir, filename))
    return vector
...
class Vector_Ops(unittest.TestCase):
    def setUp(self):
        ...
        global doublevec
        from ooflib.SWIG.common import ooferror
        from ooflib.SWIG.engine import doublevec
...
class Matrix_Factorization(unittest.TestCase):
    ## TODO: Preconditioners require a LinearizedSystem argument,
    ## which isn't actually used by any existing Preconditioner
    ## classes, so the LinearizedSystem null constructor is used here.
    ## If a future Preconditioner class requires a real
    ## LinearizedSystem, then these tests will have to be moved so
    ## that they run after the SubProblem tests.  See the comment in
    ## linearizedsystem.h.
    def setUp(self):
        global sparsemat
        ...
        from ooflib.SWIG.engine.linearizedsystem import LinearizedSystem
        self.dummyLS = LinearizedSystem()
    def LU(self):
        ...
        pc = pcwrap.create_preconditioner(matrix, self.dummyLS)
